...
const yahooApi = {
    ...
    "engines": ["indices", "forexes", "futures", "war",/* "commodities", "finances"*/],
    ...
    for (let symbol of yahooApi[engine]) {
        ...
        if (symbol !== "SKYDEX" || symbol !== "%5ECMC200" || symbol !== "%5EVIX" || symbol !== "%5EXAX" || symbol !== "%5EIPSA" || symbol !== "%5EMERV") {
            if (yahoo[engine][symbol].currency !== "USD" && yahoo[engine][symbol].currency !== "") {
                ...
                errorCheck = true;
                totalerr++;
                //console.log(engine,symbol,totalerr);
            }
            else {
    ...
    fetch(url, {
        ...
        .then((json) => {
            if (json.chart.error === null) {
                ...
                if (engine === "war" || engine === "finances" || engine === "forexes") {
                    yReport = `${symbol}`;
                }
                if (engine === "futures") {
            ...
            if (yahooReport[`${engine}Nb`] === yahooApi[engine].length) {
                ...
                if (engine === "indices" || engine === "war" || engine === "finances") {
                    getSkydex(engine);
                }
                /*if (engine === "war") {
                    getSkydex(engine);
